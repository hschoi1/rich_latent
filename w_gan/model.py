...
def model_fn(features, labels, mode, params, config):
    ...
    #     z = tf.placeholder(tf.float32, shape=(batch_size, FLAGS.z_dim))
    noise_dist = tf.contrib.distributions.Normal(0., 1.)
    z = noise_dist.sample((FLAGS.batch_size, FLAGS.z_dim))
    ...
    if FLAGS.mode is 'gp':
        ...
        alpha = alpha_dist.sample((FLAGS.batch_size, 1, 1, 1))
        ...
        c_loss += FLAGS.lam * gradient_penalty
    ...
    opt_g = ly.optimize_loss(loss=g_loss, learning_rate=FLAGS.learning_rate_ger,
                             optimizer=partial(tf.train.AdamOptimizer, beta1=0.5,
                                               beta2=0.9) if FLAGS.is_adam is True else tf.train.RMSPropOptimizer,
                             variables=theta_g, global_step=i,
                             summaries=['gradient_norm'])
    #counter_c = tf.Variable(trainable=False, initial_value=0, dtype=tf.int32)
    opt_c = ly.optimize_loss(loss=c_loss, learning_rate=FLAGS.learning_rate_dis,
                             optimizer=partial(tf.train.AdamOptimizer, beta1=0.5,
                                               beta2=0.9) if FLAGS.is_adam is True else tf.train.RMSPropOptimizer,
                             variables=theta_c, global_step=i,
                             summaries=['gradient_norm'])
    if FLAGS.mode is 'regular':
        clipped_var_c = [tf.assign(var, tf.clip_by_value(var, FLAGS.clamp_lower, FLAGS.clamp_upper)) for var in theta_c]
        # merge the clip operations on critic variables
        with tf.control_dependencies([opt_c]):
            opt_c = tf.tuple(clipped_var_c)
    if not FLAGS.mode in ['gp', 'regular']:
        raise (NotImplementedError('Only two modes'))



    citers = tf.cond(tf.logical_or(tf.less(i, tf.constant(25, dtype=tf.int64)),
                                   tf.equal(tf.divide(tf.train.get_global_step(), tf.constant(500, dtype=tf.int64)), tf.zeros([],dtype=tf.float64))),
                     lambda: tf.constant(100, dtype=tf.int64), lambda: tf.constant(FLAGS.Citers, dtype=tf.int64))


    train_op = tf.cond(tf.equal(tf.divide(tf.train.get_global_step(), citers), tf.zeros([],dtype=tf.float64)),
                       lambda: opt_c, lambda: opt_g)
    loss = tf.cond(tf.equal(tf.divide(tf.train.get_global_step(), citers), tf.zeros([],dtype=tf.float64)),
