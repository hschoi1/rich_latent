...
def model_fn(features, labels, mode, params, config):
    ...
    # for visualization of 2 dim latent space for each digit
    if mode == tf.estimator.ModeKeys.PREDICT:

        # visualize normal digit
        visualized_posterior_samples = approx_posterior.sample(100)
        inversed_samples = latent_prior.bijector.inverse(visualized_posterior_samples)
        predictions['inversed_posterior_samples'] = inversed_samples
        predictions['visualized_posterior_samples'] = visualized_posterior_samples

        # visualize adversarially perturbed noise
        adv_approx_posterior = encoder(adversarial_example)
        adv_visualized_posterior_samples = adv_approx_posterior.sample(100)
        adv_inversed_samples = latent_prior.bijector.inverse(adv_visualized_posterior_samples)
        predictions['adv_inversed_posterior_samples'] = adv_inversed_samples
        predictions['adv_visualized_posterior_samples'] = adv_visualized_posterior_samples

        # get elbo for adversarially perturbed noise
        adv_approx_posterior_sample = adv_approx_posterior.sample(params["n_samples"])
        adv_decoder_likelihood = decoder(adv_approx_posterior_sample)
        adv_distortion = -adv_decoder_likelihood.log_prob(adversarial_example)
        adv_rate = (adv_approx_posterior.log_prob(adv_approx_posterior_sample)
                    - latent_prior.log_prob(adv_approx_posterior_sample))

        adv_elbo_local = -(adv_rate + adv_distortion)
        predictions['adv_rate'] = adv_rate
        predictions['adv_distortion'] = adv_distortion
        predictions['adv_elbo'] = adv_elbo_local
