...
compare_datasets = ['cifar10','imagenet', 'celeba', 'cfiar100','svhn']
keys=['logp(x)']
bins = {'logp(x)':(-15000, 0., 150)}
...
def get_data(hps, sess):
    ...
    if hps.n_test == -1:
        hps.n_test = {'mnist': 10000, 'cifar10': 5000, 'imagenet-oord': 50000, 'imagenet': 50000,
...
def main(hps):
    ...
    hps.n_test = len(compare_datasets) * 1000
...
def infer(sess, model, hps, iterator):
    ...
    test_results = np.clip(test_results, -40000., 0.) 
    results = []
    results.append(test_results)
    plot_analysis(results, compare_datasets, keys, bins)    
