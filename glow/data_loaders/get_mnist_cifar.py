import numpy as np
import pdb
from data_loaders.get_datadata import *


noised_list = [False, False, False, False, False]
noise_type_list = ['normal', 'normal', 'normal', 'normal','normal']
compare_datasets = [tf.keras.datasets.cifar10, 'ImageNet','celebA',tf.keras.datasets.cifar100,'SVHN']
...
def get_data(problem, shards, rank, data_augmentation_level, n_batch_train, n_batch_test, n_batch_init, resolution):
    ...
    x_test = build_eval_multiple_datasets(compare_datasets, 50, noised_list=noised_list, noise_type_list=noise_type_list, feature_shape=(32,32,3))
    y_test = np.zeros([1000*len(compare_datasets)], dtype=np.int32)
