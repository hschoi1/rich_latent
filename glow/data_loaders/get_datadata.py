...
def build_eval_multiple_datasets(dataset_list, batch_size, expand_last_dim=False, noised_list=None,
                               noise_type_list=None, feature_shape=(28,28,1)):
...
def build_eval_helper(dataset, expand_last_dim=False, noised=False, noise_type='normal', feature_shape=(28,28,1)):
    ...
    if noised:
        if noise_type == 'normal':
            noise = np.random.normal(loc=0., scale=255., size=(1000, *image_shape)).astype("float32")
        elif noise_type == 'uniform':
            noise = np.random.uniform(low=0., high=255., size=(1000, *image_shape)).astype("float32")
